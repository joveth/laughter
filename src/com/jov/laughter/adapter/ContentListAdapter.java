...
public class ContentListAdapter extends BaseAdapter {
	...
	public ContentListAdapter(Context ctx, List<GoodContentBean> list,DBOpenHelper db) {
		this.list = list;
		this.ctx = ctx;
	...
	public View getView(final int position, View arg1, ViewGroup arg2) {
		...
		final int tid = list.get(position).getCid();
		if(tid!=0&&Common.isLongStr(list.get(position).getContent(), 500)){
			hold.MainText.setText(Common.cutLongStr(list.get(position).getContent(),500));
			hold.more.setVisibility(View.VISIBLE);
		}else{
			hold.MainText.setText(list.get(position).getContent());
			hold.more.setVisibility(View.GONE);
		}
		if(!Common.isEmpty(list.get(position).getImage())){
			hold.more.setVisibility(View.VISIBLE);
		}
		hold.itemIdText.setText(String.valueOf(position+1));
		...
		hold.fav.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View arg0) {
				dbOpenHelper.insertFavoriteWithCid(tid);
				Toast.makeText(ctx,
		...
		hold.more.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				intent.putExtra("flag.type", "good");
