...
import com.jov.laughter.MainActivity;
...
public class UpdateManager {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				long downloadSize = downloadUpdateFile(down_url,
						FileUtiles.updateFile.toString());
				if (downloadSize > 0) {
					// 下载成功
	...
	private final Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case DOWN_OK:
				// 下载完成，点击安装
				Uri uri = Uri.fromFile(FileUtiles.updateFile);
				Intent intent = new Intent(Intent.ACTION_VIEW);
				intent.setDataAndType(uri,
						"application/vnd.android.package-archive");
				pendingIntent = PendingIntent
						.getActivity(context, 0, intent, 0);
				notification.setLatestEventInfo(context, "搞笑吧", "下载完成，点击安装",
						pendingIntent);
				notificationManager.notify(notification_id, notification);
				context.startActivity(intent);
				break;
			case DOWN_ERROR:
				notification.setLatestEventInfo(context, "搞笑吧", "下载失败",
						pendingIntent);
				break;
			case NEW_UPDATE:
				if (bean == null) {
